...
def DDQN_BPT(config, args, env, checkpoint_dir=None):

    agent = DDQN(args, config, checkpoint_dir)  
...
def main():
    ...
    if(args.test_only):
        ...
        agent = DDQN(args, config, best_model)
        ...
        analysis = tune.run(
            partial(DDQN_BPT, args=args, env=env),
            name="ddqn_PBT",
            scheduler=pbt,
            stop={
                "reward_mean": 290,
        ...
        agent = DDQN(args, best_trial.config, best_checkpoint_dir)
        writer = SummaryWriter(args.logdir)
        test(args, env, agent, writer)
