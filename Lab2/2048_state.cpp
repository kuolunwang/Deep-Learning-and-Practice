	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				...
				// -----------------------------------
				// return reward + sum of all posible move->after_state()
				// -----------------------------------
...
void save_score(size_t episode, int score){
	...
	std::ofstream file;
	file.open("./record_score_5.txt",std::ios::app);
...
int main(int argc, const char* argv[]) {
	...
	tdl.load("./weight/alpha_0.0001_state_100000");
	...
	tdl.save("./weight/alpha_0.0001_state_200000");
